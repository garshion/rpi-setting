...
헤드리스(SSH 전용) 서버 용도에 맞게 메모리 및 서비스를 경량화하는 세팅입니다.
...
디스플레이를 사용하지 않으므로 GPU 메모리를 최소값(16MB)으로 축소하여 시스템 메모리를 확보합니다.

### 2. Swap 1GB 생성
물리 메모리가 512MB로 제한적이므로 1GB 스왑 파일을 생성합니다.<br/>
`swappiness=10`으로 설정하여 가능한 한 RAM을 우선 사용합니다.

### 3. 불필요한 서비스 비활성화
헤드리스 서버에서 사용하지 않는 서비스를 비활성화합니다.
...
### 4. 블루투스 비활성화
블루투스를 사용하지 않으므로 하드웨어 레벨에서 비활성화합니다.
